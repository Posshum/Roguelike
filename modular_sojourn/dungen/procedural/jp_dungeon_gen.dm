...
/obj/procedural/jp_DungeonGenerator
	...
	var/maximumIterations = 350 //The number of do-nothing iterations before the generator gives up with an error.
	...
	var/regen_light = /obj/machinery/light/rogue/torchholder/autoattach
...
/obj/procedural/jp_DungeonGenerator/proc/AddLight(t)
	if(regen_specific)
		...
		new /obj/machinery/light/rogue/torchholder/autoattach(t)
...
/obj/procedural/jp_DungeonGenerator/proc/initializeSubmaps()
	var/datum/map_template/init_template = new /datum/map_template/dungeon_template/room
...
*/
/* 
/obj/procedural/jp_DungeonGenerator/proc/iswallturf(turf/t)
...
/obj/procedural/jp_DungeonGenerator/proc/generate(seed=null)
	...
	if(usePreexistingRegions)
		...
		for(var/turf/t in block(locate(getMinX(), getMinY(), getZ()), locate(getMaxX(), getMaxY(), getZ())))
			if(!iswallturf(t)) if(!(t in examined)) rooms+=regionCreate(t)
	...
	for(var/turf/t in border_turfs)
		for(var/turf/t2 in range(t, 1))
			if(iswallturf(t2)&&!(t2 in border_turfs))
	...
	initializeSubmaps()
	testing("There has been a total of [debugincrementer] templates generated.")
	...
	while(1)
		...
		for(var/turf/t in getAdjacent(min))
			...
			for(var/turf/t1 in GetSquare(t, pathWidth + 1))
				if(!(iswallturf(t1) && !(t1 in borders)))
					stop_looking = TRUE
					break
			if(stop_looking)
				continue
		...
		if(!(t in contained))
			...
			if(!noborder)
				for(var/turf/t2 in gen.getAdjacent(t))
					if(iswallturf(t2) && !(t2 in border)) border+=t2
...
to the floor that return true from iswallturf().
*/
...
/obj/procedural/jp_DungeonRoom/preexist/cross/New()
	...
	for(var/turf/t in range(centre, size+1))
		...
		if(iswallturf(t) && (t.x == getX() || t.y == getY()))
			border+=t
